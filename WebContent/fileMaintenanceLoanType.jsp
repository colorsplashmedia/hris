...
<script type="text/javascript">
    ...
    $(document).ready(function () {
    	...
    	<c:forEach var="fm" items="${sessionScope.moduleAccess.fileManagementList}">
			<c:if test="${fm == 'fm_loan_type'}">
				isAllowed = 'YES';
			</c:if>
			<c:if test="${fm == 'fm_add_loan_type'}">
				addButtonEnabled = 'YES';
			</c:if>
			
			<c:if test="${fm == 'fm_edit_loan_type'}">
				editButtonEnabled = 'YES';
			</c:if>
			
			<c:if test="${fm == 'fm_delete_loan_type'}">
				deleteButtonEnabled = 'YES';
			</c:if>
			
			<c:if test="${fm == 'fm_print_loan_type'}">
				printButtonEnabled = 'YES';
			</c:if>
			
			<c:if test="${fm == 'fm_export_loan_type'}">
    	...
    	if(addButtonEnabled == 'YES' && editButtonEnabled == 'YES' && deleteButtonEnabled == 'YES'){    		
    		actionsJTableVar = {    	    		    	    	
				...
				deleteAction: '/hris/DeleteLoanTypeAction'
    	    };
        ...
        $('#containerLoanType').jtable({
            ...
            fields: {
                ...
                institution: {
                    title: 'Institution',
                    edit : true,
                    options:  [{ Value: 'GSIS', DisplayText: 'GSIS' }, { Value: 'PAGIBIG', DisplayText: 'PAGIBIG' }, { Value: 'PHILHEALTH', DisplayText: 'PHILHEALTH' }, { Value: 'OTHERS', DisplayText: 'OTHERS' }]
                },
                accountingCode: {
...
<div id="content">
	...
	<div class="filtering" style="width: 574px;">
        ...
        Loan Type Name: &nbsp;&nbsp;&nbsp;<input type="text" name="name" id="name" size="40" onKeyPress="javascript:enterPressFunc(event);" />        
