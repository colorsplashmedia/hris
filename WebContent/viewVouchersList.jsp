...
<html>
<head>
<title>Disbursement Voucher List</title>
...
<script>
	...
	$(document).ready(function() {
		...
		var oAjaxCall = $.ajax({
			type : "POST",
			url : "GetDisbursementVoucherListAction",
			cache : false,
			async : false,
			dataType : "json",
			success : function(data) {
				...
				items += "<thead>";
				items += "<tr>";
				items += "<th scope='col'>Disbursement Date</th>";
				...
				if (data.Records.length) {
					$.each(data.Records, function(i, item) {
						...
						items += "<td>" + item.disbursementDate + "</td>";
						items += "<td>" + item.payee + "</td>";
						items += "<td>" + item.particulars + "</td>";
						items += "<td>" + item.totalAmount + "</td>";						
						items += "<td><img src='images/edit_blue.png' alt='' width='30px' height='30px' onclick='editVoucher(" + item.disbursementVoucherId + ");' /></td>";
	...
	function editVoucher(disbursementVoucherId) {
		window.location = "disbursementVoucherEdit.jsp?disbursementVoucherId=" + disbursementVoucherId;
		...
		<div style="width: 820px; margin: 0 auto;">
				<div id="clockEntryArea">					
					<div class="headerDashboardTable" style="float: left;">					
						...
						<input id="addEmployeeButton" value="Add Disbursement Voucher" border="0" onclick="javascript:window.location='disbursementVoucherEntry.jsp';">					
