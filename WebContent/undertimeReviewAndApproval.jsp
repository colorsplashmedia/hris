...
<html>
<head>
<title>Employee Undertime Approval</title>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
    	...
    	var actionsJTableVar = {    			
    		listAction: '/hris/GetAllUndertimeSvApprovalAction?superVisorId='+superVisorId//,                
    		//updateAction: '/hris/UpdateUndertimeSvApprovalAction?superVisorId='+superVisorId
    	};    	
        ...
        $('#containerEmployeeUndertimeEntry').jtable({
            title: 'Employee Undertime Approval',
            paging: true,
            pageSize: 10,
            sorting: true,
            ...
            fields: {
                ...
                approvedBy: {
                	...
                	display: function(data) {
                         return '<div style="width: 170px;"><button type="button" style="margin-left: 7px;" onclick="approveUndertime(' + data.record.empUndertimeId +')"> Approve</button> <button type="button" style="margin-left: 7px;" onclick="disapproveUndertime(' + data.record.empUndertimeId + ')"> Disapprove</button></div> ';
                    } 
        ...
        $('#containerEmployeeUndertimeEntry').jtable('load');
		 ...
		 function approveUndertime(empUndertimeId){
				...
				if(status == 0){
					...
					if (confirm('Are you sure you want to approve the Undertime Entry?')) {
						$.ajax({
							type:"POST",
							url:"/hris/UpdateUndertimeSvApprovalAction?empUndertimeId="+empUndertimeId+"&approveFlag=Y",			
							async: true,
							dataType: 'json',
							success: function (data) {				
								if(data.Record == "YES") {
									alert("Undertime Entry has been approved.");
									location.reload();
								}							
							},
		 ...
		 function disapproveUndertime(empUndertimeId){
				...
				if(status == 0){
					...
					if (confirm('Are you sure you want to disapprove the Undertime Entry?')) {
						$.ajax({
							type:"POST",
							url:"/hris/UpdateUndertimeSvApprovalAction?empUndertimeId="+empUndertimeId+"&approveFlag=N",			
							async: true,
							dataType: 'json',
							success: function (data) {				
								if(data.Record == "YES") {
									alert("Undertime Entry has been disapproved.");
									location.reload();
								}							
							},
							error: function (data) {alert('error: '+data)}
