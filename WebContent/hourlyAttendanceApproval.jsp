...
<html>
<head>
<title>Employee Hourly Attendance Approval</title>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
    	...
    	var actionsJTableVar = {    			
    		listAction: '/hris/GetAllHourlyAttendanceSvApprovalAction'
    	};    	
        ...
        $('#containerEmployeeHourlyAttendanceEntry').jtable({
            title: 'Employee Hourly Attendance Approval',
            paging: true,
            pageSize: 10,
            sorting: true,
            ...
            formCreated : function(event, data){
	            ...
	            $dialogDiv.position({
	                my: "center",
	                at: "center",
	                of: $("#containerEmployeeHourlyAttendanceEntry")
	            });
            ...
            fields: {
                ...
                attendanceDate: {
                    ...
                    display:
                      function (data) {
                    	if (data.record.attendanceDate != undefined){				//need to have a condition for undefined since moment will display default date.
                         return moment(data.record.attendanceDate).format('YYYY-MM-DD');
                    	}
                    	return 'N/A';
                      }
                },
                ...
                approvedBy: {
                	...
                	display: function(data) {
                         return '<div style="width: 170px;"><button type="button" style="margin-left: 7px;" onclick="approveHourlyAttendance(' + data.record.empHourlyAttendanceId +')"> Approve</button> <button type="button" style="margin-left: 7px;" onclick="disapproveHourlyAttendance(' + data.record.empHourlyAttendanceId + ')"> Disapprove</button></div> ';
                    } 
        ...
        $('#containerEmployeeHourlyAttendanceEntry').jtable('load');
        
    });
    
		 function approveHourlyAttendance(empHourlyAttendanceId){
				...
				if(status == 0){
					...
					if (confirm('Are you sure you want to approve the Hourly Attendance Entry?')) {
						$.ajax({
							type:"POST",
							url:"/hris/UpdateHourlyAttendanceSvApprovalAction?empHourlyAttendanceId="+empHourlyAttendanceId+"&approveFlag=Y",			
							async: true,
							dataType: 'json',
							success: function (data) {				
								if(data.Record == "YES") {
									alert("Hourly Attendance Entry has been approved.");
									location.reload();
								}							
							},
		 ...
		 function disapproveHourlyAttendance(empHourlyAttendanceId){
				...
				if(status == 0){
					...
					if (confirm('Are you sure you want to disapprove the Hourly Attendance Entry?')) {
						$.ajax({
							type:"POST",
							url:"/hris/UpdateHourlyAttendanceSvApprovalAction?empHourlyAttendanceId="+empHourlyAttendanceId+"&approveFlag=N",			
							async: true,
							dataType: 'json',
							success: function (data) {				
								if(data.Record == "YES") {
									alert("Hourly Attendance Entry has been disapproved.");
									location.reload();
								}							
							},
							error: function (data) {alert('error: '+data)}
...
<div class="cb" style="height: 20px;"></div>
	<div id="containerEmployeeHourlyAttendanceEntry" class="jTableContainerDaiExtended"></div>		
</div>	
