...
<html>
<head>
<title>Employee OffSet Approval</title>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
    	...
    	var actionsJTableVar = {    			
    		listAction: '/hris/GetAllOffSetSvApprovalAction?superVisorId='+superVisorId//,                
    		//updateAction: '/hris/UpdateOffSetSvApprovalAction?superVisorId='+superVisorId
    	};    	
        ...
        $('#containerEmployeeOffSetEntry').jtable({
            title: 'Employee OffSet Approval',
            paging: true,
            pageSize: 10,
            sorting: true,
            ...
            fields: {
                ...
                approvedBy: {
                	...
                	display: function(data) {
                         return '<div style="width: 170px;"><button type="button" style="margin-left: 7px;" onclick="approveOffSet(' + data.record.empOffSetId +')"> Approve</button> <button type="button" style="margin-left: 7px;" onclick="disapproveOffSet(' + data.record.empOffSetId + ')"> Disapprove</button></div> ';
                    } 
        ...
        $('#containerEmployeeOffSetEntry').jtable('load');
		 ...
		 function approveOffSet(empOffSetId){
				...
				if(status == 0){
					...
					if (confirm('Are you sure you want to approve the OffSet Entry?')) {
						$.ajax({
							type:"POST",
							url:"/hris/UpdateOffSetSvApprovalAction?empOffSetId="+empOffSetId+"&approveFlag=Y",			
							async: true,
							dataType: 'json',
							success: function (data) {				
								if(data.Record == "YES") {
									alert("OffSet Entry has been approved.");
									location.reload();
								}							
							},
		 ...
		 function disapproveOffSet(empOffSetId){
				...
				if(status == 0){
					...
					if (confirm('Are you sure you want to disapprove the OffSet Entry?')) {
						$.ajax({
							type:"POST",
							url:"/hris/UpdateOffSetSvApprovalAction?empOffSetId="+empOffSetId+"&approveFlag=N",			
							async: true,
							dataType: 'json',
							success: function (data) {				
								if(data.Record == "YES") {
									alert("OffSet Entry has been disapproved.");
									location.reload();
								}							
							},
							error: function (data) {alert('error: '+data)}
