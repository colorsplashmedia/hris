...
<script type="text/javascript">
    ...
    $(document).ready(function () {
    	...
    	<c:forEach var="fm" items="${sessionScope.moduleAccess.fileManagementList}">
			<c:if test="${fm == 'fm_country'}">
				isAllowed = 'YES';
			</c:if>
			<c:if test="${fm == 'fm_add_country'}">
				addButtonEnabled = 'YES';
			</c:if>
			
			<c:if test="${fm == 'fm_edit_country'}">
				editButtonEnabled = 'YES';
			</c:if>
			
			<c:if test="${fm == 'fm_delete_country'}">
				deleteButtonEnabled = 'YES';
			</c:if>
			
			<c:if test="${fm == 'fm_print_country'}">
				printButtonEnabled = 'YES';
			</c:if>
			
			<c:if test="${fm == 'fm_export_country'}">
    	...
    	if(addButtonEnabled == 'YES' && editButtonEnabled == 'YES' && deleteButtonEnabled == 'YES'){    		
    		actionsJTableVar = {    	    		    	    	
    			listAction: '/hris/GetAllCountryAction',                
    	        createAction: '/hris/AddCountryAction',
				updateAction: '/hris/UpdateCountryAction',
				deleteAction: '/hris/DeleteCountryAction'
    	    };
    		...
    		toolBarsJTableVar = {    	    		    	    	
    				items: [{
                        ...
                        click: function () {
                        	var oAjaxCall = $.ajax({
	                    		type : "POST",
	                    		url : "/hris/ActionPdfExportFMCountryListReport",	                    		
                        ...
                        text: 'Print to PDF',
                        click: function () {
                        	window.open('ActionPdfExportFMCountryListReport?name=' + $('#name').val()+"&forExport=N", '_blank');
                        }
                    }]   			    		
    	    };
...
<div id="content">
	...
	<div class="filtering" style="width: 574px;">
        ...
        Country Name: &nbsp;&nbsp;&nbsp;<input type="text" name="name" id="name" size="40" onKeyPress="javascript:enterPressFunc(event);" />        
        
        <div style="cursor: pointer;" id="searchButton">SEARCH</div>
    
	</div>
	<div id="containerCountry"  class="jTableContainerDai"></div>
</div>	
