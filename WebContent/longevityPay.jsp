...
<script type="text/javascript">
    ...
    function clickSearchResult(empid) {
    	...
    	<c:forEach var="fm" items="${sessionScope.moduleAccess.payrollList}">
			...
			<c:if test="${fm == 'py_add_longevity_pay'}">
				addButtonEnabled = 'YES';
			</c:if>
			
			<c:if test="${fm == 'py_edit_longevity_pay'}">
				editButtonEnabled = 'YES';
			</c:if>
			
			<c:if test="${fm == 'py_print_longevity_pay'}">
				printButtonEnabled = 'YES';
			</c:if>
			
			<c:if test="${fm == 'py_export_longevity_pay'}">
		...
		if(addButtonEnabled == 'YES' && editButtonEnabled == 'YES'){    		
    		actionsJTableVar = {    	    		    	    	
    			listAction: '/hris/GetLongevityPayAction?toRecipientEmpId='+empid,                
    	        //createAction: '/hris/SaveLongivityPayAction',
				updateAction: '/hris/SaveLongivityPayAction'    			    		
    	    };
    		...
    		toolBarsJTableVar = {    	    		    	    	
    				items: [{
                        ...
                        click: function () {
                        	var oAjaxCall = $.ajax({
	                    		type : "POST",
	                    		url : "/hris/ActionPdfExportLongevityPayListReport",	                    		
                        ...
                        text: 'Print to PDF',
                        click: function () {
                        	window.open('ActionPdfExportLongevityPayListReport?forExport=N'+'&empId='+empid, '_blank');
                        }
                    }]   			    		
    	    };
		...
		$('#containerLongivityPay').jtable();
		$('#containerLongivityPay').jtable('destroy');
		$('#containerLongivityPay').jtable({
			title: 'Longevity Pay',
		...
		$('#containerLongivityPay').jtable('load');
    }
</script>
...
<div id="content">
	...
	<div id="dashBoardRightPannel2" width="100%">	
		...
		<div id="containerLongivityPay" class="jTableContainerDaiLeft1020"></div>		
	</div>
</div>
