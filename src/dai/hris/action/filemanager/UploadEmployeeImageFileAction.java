     ...
     try {
         ...
         for (FileItem item : items) {
             if (!item.isFormField()) {
                     ...
                     String absolutePathToIndexJSP = context.getRealPath("/empImages/");
                     
                     fileName = empId + "." + FilenameUtils.getExtension(item.getName());
                     File file = new File(absolutePathToIndexJSP, fileName);  //FilenameUtils.getExtension(filename)
                     File fileDirecotry = new File(absolutePathToIndexJSP);
