...
/**
 * Servlet implementation class ActionPdfExportFMCProfessionalFeetListReport
 */
@WebServlet("/ActionPdfExportFMProfessionalFeeListReport")
public class ActionPdfExportFMProfessionalFeeListReport extends HttpServlet {
	private static final long serialVersionUID = 5676921778006010751L;
	private static Logger logger = Logger.getLogger(ActionPdfExportFMProfessionalFeeListReport.class);
	Gson gson = new Gson();
...
//			}
			...
			List<ProfessionalFee> dataList = new ArrayList<ProfessionalFee>();	
...
//			}
			...
			dataList = service.getProfessionalFeeListByEmpId(empId, startPageIndex,numRecordsPerPage, sorting);
			
			if(dataList.isEmpty()){
				
				ProfessionalFee model = new ProfessionalFee();
				
				model.setProfessionalFeeId(0);
				model.setOfficialReceiptNumber("");
				model.setOfficialReceiptDate("");
				model.setGrossAmount(BigDecimal.ZERO);
				model.setWithHoldingTax(BigDecimal.ZERO);
				model.setFinalTax(BigDecimal.ZERO);
			...
			if(forExport.equals("Y")) {
				...
				URL sourceFileName = getClass().getResource("/dai/hris/reports/ProfessionalFeeReport.jasper");
				...
				String reportName = "ProfessionalFeeListReport" + "_" + employeeLoggedIn.getSectionId() 
