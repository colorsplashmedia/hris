 ...
 */
@WebServlet("/ActionPdfExportContributionHDMFReport")
public class ActionPdfExportContributionHDMFReport extends HttpServlet {
	private static final long serialVersionUID = 5676921778006010751L;
	private static Logger logger = Logger.getLogger(ActionPdfExportContributionHDMFReport.class);
	Gson gson = new Gson();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		InputStream reportStream = getClass().getClassLoader().getResourceAsStream( "dai/hris/reports/HDMFReport.jasper");
		...
		try {
			...
			map.put("REPORT_NAME", "PAGIBIG CONTRIBUTION REPORT");
			...
			if(forIndividual.equals("Y")) {
				...
				dataList = service.getHDMFReportByDateRangeAndId(employeeLoggedIn.getEmpId(), dateFrom, dateTo);
			} else {
				String payrollPeriodIdStr = request.getParameter("payrollPeriodId");
				dataList = service.getHDMFReport(Integer.parseInt(payrollPeriodIdStr));		
			}
			...
			if(forExport.equals("Y")) {
				...
				URL sourceFileName = getClass().getResource("/dai/hris/reports/HDMFReport.jasper");
				...
				String reportName = "hdmfContributionListReport" + "_" + employeeLoggedIn.getSectionId() 
				...
				if(forExport.equals("Y")) {
					sysModel.setProcessDesc("EXPORT HDMF CONTRIBUTION REPORT");
					sysModel.setProcessType("EXPORT");
				} else {
					sysModel.setProcessDesc("VIEW PDF HDMF CONTRIBUTION REPORT");
					sysModel.setProcessType("VIEW PDF");
				}
				
				sysModel.setModuleName("HDMF CONTRIBUTION REPORT");
