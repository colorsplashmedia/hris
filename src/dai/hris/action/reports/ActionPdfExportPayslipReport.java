 ...
 */
@WebServlet("/ActionPdfExportPayslipReport")
public class ActionPdfExportPayslipReport extends HttpServlet {
	private static final long serialVersionUID = 5676921778006010751L;
	private static Logger logger = Logger.getLogger(ActionPdfExportPayslipReport.class);
	Gson gson = new Gson();
...
//		InputStream reportStream = getClass().getClassLoader().getResourceAsStream( "dai/hris/reports/Payslip.jasper");
		InputStream reportStream = getClass().getClassLoader().getResourceAsStream( "dai/hris/reports/PayslipZamboanga.jasper");
		...
		try {
			...
			dataList = service.getPaylipByEmpId(empIdList, Integer.parseInt(payrollPeriodIdStr));		
