 ...
 */
@WebServlet("/ActionPdfExportHazardPayListReport")
public class ActionPdfExportHazardPayListReport extends HttpServlet {
	private static final long serialVersionUID = 5676921778006010751L;
	private static Logger logger = Logger.getLogger(ActionPdfExportHazardPayListReport.class);
	Gson gson = new Gson();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		InputStream reportStream = getClass().getClassLoader().getResourceAsStream( "dai/hris/reports/HazardPayReport.jasper");
		...
		try {
			...
			List<HazardPay> dataList = new ArrayList<HazardPay>();	
						
			dataList = service.getHazardPayListByEmpId(empId, startPageIndex,numRecordsPerPage, sorting);
			...
			if(forExport.equals("Y")) {
				...
				URL sourceFileName = getClass().getResource("/dai/hris/reports/HazardPayReport.jasper");
				...
				String reportName = "hazardPayListReport" + "_" + employeeLoggedIn.getSectionId() 
				...
				if(forExport.equals("Y")) {
					sysModel.setProcessDesc("EXPORT HAZARD PAY REPORT");
					sysModel.setProcessType("EXPORT");
				} else {
					sysModel.setProcessDesc("VIEW PDF HAZARD PAY REPORT");
					sysModel.setProcessType("VIEW PDF");
				}
				
				sysModel.setModuleName("HAZARD PAY REPORT");
