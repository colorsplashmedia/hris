...
/**
 * Servlet implementation class ActionPdfExportOtherLoansRemittanceReport
 */
@WebServlet("/ActionPdfExportOtherLoansRemittanceReport")
public class ActionPdfExportOtherLoansRemittanceReport extends HttpServlet {
	private static final long serialVersionUID = 5676921778006010751L;
	private static Logger logger = Logger.getLogger(ActionPdfExportOtherLoansRemittanceReport.class);
	Gson gson = new Gson();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		InputStream reportStream = getClass().getClassLoader().getResourceAsStream( "dai/hris/reports/OtherLoanMonthlyRemittanceReport.jasper");
		...
		ILoanTypeService loanService = new LoanTypeService();
		...
		try {
			...
			LoanType loanType = loanService.getLoanType(loanTypeId);
			
			map.put("REPORT_NAME", loanType.getLoanTypeName().toUpperCase());	
			map.put("PERIOD", monthString + " " + year);	
			
			List<OtherLoanRemittance> dataList = new ArrayList<OtherLoanRemittance>();	
			
			dataList = service.getOtherLoanRemittanceReport(loanTypeId, month, year);	
			...
			if(forExport.equals("Y")) {
				...
				URL sourceFileName = getClass().getResource("/dai/hris/reports/OtherLoanMonthlyRemittanceReport.jasper");
				
				printFileName = JasperFillManager.fillReportToFile(sourceFileName.toURI().getPath(), map, beanColDataSource);
				
				SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss");
	            Calendar calendar = Calendar.getInstance();
				String reportName = "otherLoansRemittanceReport" + "_" + employeeLoggedIn.getSectionId() 
				...
				if(forExport.equals("Y")) {
					sysModel.setProcessDesc("EXPORT OTHER LOANS REMITTANCE REPORT");
					sysModel.setProcessType("EXPORT");
				} else {
					sysModel.setProcessDesc("VIEW PDF OTHER LOANS REMITTANCE REPORT");
