 ...
 */
@WebServlet("/ActionPdfExportContributionPHICReport")
public class ActionPdfExportContributionPHICReport extends HttpServlet {
	private static final long serialVersionUID = 5676921778006010751L;
	private static Logger logger = Logger.getLogger(ActionPdfExportContributionPHICReport.class);
	Gson gson = new Gson();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		InputStream reportStream = getClass().getClassLoader().getResourceAsStream( "dai/hris/reports/PHICReport.jasper");
		...
		try {
			...
			map.put("REPORT_NAME", "PHIC CONTRIBUTION REPORT");
			...
			if(forIndividual.equals("Y")) {
				...
				dataList = service.getPHICReportByDateRangeAndId(employeeLoggedIn.getEmpId(), dateFrom, dateTo);
			} else {
				String payrollPeriodIdStr = request.getParameter("payrollPeriodId");
				dataList = service.getPHICReport(Integer.parseInt(payrollPeriodIdStr));		
			}
			...
			if(forExport.equals("Y")) {
				...
				URL sourceFileName = getClass().getResource("/dai/hris/reports/PHICReport.jasper");
				...
				String reportName = "phicContributionListReport" + "_" + employeeLoggedIn.getSectionId() 
				...
				if(forExport.equals("Y")) {
					sysModel.setProcessDesc("EXPORT PHIC CONTRIBUTION REPORT");
					sysModel.setProcessType("EXPORT");
				} else {
					sysModel.setProcessDesc("VIEW PDF PHIC CONTRIBUTION REPORT");
					sysModel.setProcessType("VIEW PDF");
				}
				
				sysModel.setModuleName("PHIC CONTRIBUTION REPORT");
