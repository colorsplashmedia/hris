 ...
 */
@WebServlet("/ActionPdfExportContributionGSISReport")
public class ActionPdfExportContributionGSISReport extends HttpServlet {
	private static final long serialVersionUID = 5676921778006010751L;
	private static Logger logger = Logger.getLogger(ActionPdfExportContributionGSISReport.class);
	Gson gson = new Gson();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		InputStream reportStream = getClass().getClassLoader().getResourceAsStream( "dai/hris/reports/GSISReport.jasper");
		...
		try {
			...
			map.put("REPORT_NAME", "GSIS CONTRIBUTION REPORT");
			...
			if(forIndividual.equals("Y")) {
				...
				dataList = service.getGSISReportByDateRangeAndId(employeeLoggedIn.getEmpId(), dateFrom, dateTo);
			} else {
				String payrollPeriodIdStr = request.getParameter("payrollPeriodId");
				dataList = service.getGSISReport(Integer.parseInt(payrollPeriodIdStr));		
			}
			...
			if(forExport.equals("Y")) {
				...
				URL sourceFileName = getClass().getResource("/dai/hris/reports/GSISReport.jasper");
				...
				String reportName = "gsisContributionListReport" + "_" + employeeLoggedIn.getSectionId() 
				...
				if(forExport.equals("Y")) {
					sysModel.setProcessDesc("EXPORT GSIS CONTRIBUTION REPORT");
					sysModel.setProcessType("EXPORT");
				} else {
					sysModel.setProcessDesc("VIEW PDF GSIS CONTRIBUTION REPORT");
					sysModel.setProcessType("VIEW PDF");
				}
				
				sysModel.setModuleName("GSIS CONTRIBUTION REPORT");
