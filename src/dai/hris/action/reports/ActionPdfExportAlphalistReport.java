 ...
 */
@WebServlet("/ActionPdfExportAlphalistReport")
public class ActionPdfExportAlphalistReport extends HttpServlet {
	private static final long serialVersionUID = 5676921778006010751L;
	private static Logger logger = Logger.getLogger(ActionPdfExportAlphalistReport.class);
	Gson gson = new Gson();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		InputStream reportStream = getClass().getClassLoader().getResourceAsStream( "dai/hris/reports/AlphaListReport.jasper");
		...
		try {
			...
			List<AlphaListReport> dataList = new ArrayList<AlphaListReport>();	
			
	
			dataList = service.getBIRAlphaListReport(Integer.parseInt(year));			
			...
			if(forExport.equals("Y")) {
				...
				URL sourceFileName = getClass().getResource("/dai/hris/reports/AlphaListReport.jasper");
				...
				String reportName = "alphaListReport" + "_" + employeeLoggedIn.getSectionId() 
				...
				if(forExport.equals("Y")) {
					sysModel.setProcessDesc("EXPORT ALPHALIST REPORT");
					sysModel.setProcessType("EXPORT");
				} else {
					sysModel.setProcessDesc("VIEW PDF ALPHALIST REPORT");
					sysModel.setProcessType("VIEW PDF");
				}
				
				sysModel.setModuleName("PAYROLL ALPHALIST REPORT");
