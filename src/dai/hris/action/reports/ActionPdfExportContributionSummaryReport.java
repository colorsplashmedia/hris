 ...
 */
@WebServlet("/ActionPdfExportContributionSummaryReport")
public class ActionPdfExportContributionSummaryReport extends HttpServlet {
	private static final long serialVersionUID = 5676921778006010751L;
	private static Logger logger = Logger.getLogger(ActionPdfExportContributionSummaryReport.class);
	Gson gson = new Gson();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		InputStream reportStream = getClass().getClassLoader().getResourceAsStream( "dai/hris/reports/TotalContributionReport.jasper");
		...
		try {
			...
			map.put("REPORT_NAME", "CONTRIBUTION SUMMARY REPORT");

			
			IReportService service = new ReportService();			
			
			List<ContributionSummaryReport> dataList = new ArrayList<ContributionSummaryReport>();				
			dataList = service.getContributionSummaryReportByDateRangeAndId(employeeLoggedIn.getEmpId(), dateFrom, dateTo);		
			...
			if(forExport.equals("Y")) {
				...
				URL sourceFileName = getClass().getResource("/dai/hris/reports/TotalContributionReport.jasper");
				...
				String reportName = "contributionSummaryReport" + "_" + employeeLoggedIn.getSectionId() 
				...
				if(forExport.equals("Y")) {
					sysModel.setProcessDesc("EXPORT CONTRIBUTION SUMMARY REPORT");
					sysModel.setProcessType("EXPORT");
				} else {
					sysModel.setProcessDesc("VIEW PDF CONTRIBUTION SUMMARY REPORT");
					sysModel.setProcessType("VIEW PDF");
				}
				
				sysModel.setModuleName("CONTRIBUTION SUMMARY REPORT");
