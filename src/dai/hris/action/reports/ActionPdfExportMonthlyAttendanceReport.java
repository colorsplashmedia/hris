...
/**
 * Servlet implementation class ActionPdfExportMonthlyAttendanceReport
 */
@WebServlet("/ActionPdfExportMonthlyAttendanceReport")
public class ActionPdfExportMonthlyAttendanceReport extends HttpServlet {
	private static final long serialVersionUID = 5676921778006010751L;
	private static Logger logger = Logger.getLogger(ActionPdfExportMonthlyAttendanceReport.class);
	Gson gson = new Gson();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String reportStreamPath = "dai/hris/reports/MonthAttendanceReport.jasper";
		String exportName = "monthlyAttendanceReport";
		...
		try {
			...
			map.put("REPORT_NAME", "MONTHLY REPORT OF ATTENDANCE");
			...
			map.put("signatory2", signatory1 != null ? signatory2 : "");
			map.put("position2", position1 != null ? position2 : "");
			map.put("signatory3", signatory1 != null ? signatory3 : "");
			map.put("position3", position1 != null ? position3 : "");
			
			List<MonthlyAttendanceReport> dataList = new ArrayList<MonthlyAttendanceReport>();	
			
			dataList = service.getMonthlyAttendanceReport(empIdList, month, year);	
			
			int numberOfExtra = 0;
			if(dataList.size() < 10) {
				numberOfExtra = 10 - dataList.size();
			} else {
				int remainder = dataList.size() % 16;				
				numberOfExtra = 10 - remainder;
			}
			
			for(int x = 0; x < numberOfExtra; x++){
				MonthlyAttendanceReport model = new MonthlyAttendanceReport();
				model.setEmpName("");
