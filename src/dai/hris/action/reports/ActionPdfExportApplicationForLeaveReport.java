...
/**
 * Servlet implementation class ActionPdfExportApplicationForLeaveReport
 */
@WebServlet("/ActionPdfExportApplicationForLeaveReport")
public class ActionPdfExportApplicationForLeaveReport extends HttpServlet {
	private static final long serialVersionUID = 5676921778006010751L;
	private static Logger logger = Logger.getLogger(ActionPdfExportApplicationForLeaveReport.class);
	Gson gson = new Gson();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String reportStreamPath = "dai/hris/reports/ApplicationForLeave.jasper";
		String exportName = "applicationForLeaveLeaveReport";
		...
		String commutation = request.getParameter("commutation");	
		String noOfDays = request.getParameter("noOfDays");	
		String specify1 = request.getParameter("specify1");	
		String specify2 = request.getParameter("specify2");	
		...
		map.put("REPORT_NAME", "APPLICATION FOR LEAVE");	
		...
		map.put("signatory3", signatory2 != null ? signatory3 : "");
		
		map.put("location", location != null ? location : "");
		map.put("commutation", commutation != null ? commutation : "");
		map.put("noOfDays", noOfDays != null ? noOfDays : "");
		map.put("specify1", specify1 != null ? specify1 : "");
		map.put("specify2", specify2 != null ? specify2 : "");
		map.put("duration", dateFrom + " to " + dateTo);
		
		map.put("lastName", employeeLoggedIn.getLastname());
		map.put("firstName", employeeLoggedIn.getFirstname());
		map.put("middleName", employeeLoggedIn.getMiddlename());
		
		IJobTitleService jobService = new JobTitleService();
		IEmployeePayrollInfoService empPayrollService = new EmployeePayrollInfoService();
		
		
				
		try {
			
			JobTitle jobTitle = jobService.getJobTitle(employeeLoggedIn.getJobTitleId());
			
			EmployeePayrollInfo empPayInfo = empPayrollService.getEmployeePayrollInfo(employeeLoggedIn.getEmpId());
			
			map.put("empPosition", jobTitle.getJobTitle());
			map.put("monthlySalary", empPayInfo.getMonthlyRate());
