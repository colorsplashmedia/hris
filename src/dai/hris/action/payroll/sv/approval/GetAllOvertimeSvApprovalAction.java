...
//			list = service.getAllOvertimeForSvApprovalBySuperVisorId(supervisorId, startPageIndex,numRecordsPerPage, sorting);
			...
			if("ADMIN".equals(approverType)){
				//get all Leave
				list = service.getAllOvertimeForApproval(employeeLoggedIn.getEmpId(), startPageIndex,numRecordsPerPage, sorting);
			} else if("HRADMIN".equals(approverType)) {
				//get all Leave base on sectionId
				list = service.getAllOvertimeForHRApproval(employeeLoggedIn.getEmpId(), startPageIndex,numRecordsPerPage, sorting);
			} else if("SECTIONADMIN".equals(approverType)) {
				//get all Leave base on sectionId
				list = service.getAllOvertimeForApprovalBySectionId(employeeLoggedIn.getEmpId(), employeeLoggedIn.getSectionId(), startPageIndex,numRecordsPerPage, sorting);
			} else if("UNITADMIN".equals(approverType)) {
				//get all Leave base on unitId
				list = service.getAllOvertimeForApprovalByUnitId(employeeLoggedIn.getEmpId(), employeeLoggedIn.getUnitId(), startPageIndex,numRecordsPerPage, sorting);
			} else if("SUBUNITADMIN".equals(approverType)) {
				//get all Leave base on subUnitId			
				list = service.getAllOvertimeForApprovalBySubUnitId(employeeLoggedIn.getEmpId(), employeeLoggedIn.getSubUnitId(), startPageIndex,numRecordsPerPage, sorting);
			}
