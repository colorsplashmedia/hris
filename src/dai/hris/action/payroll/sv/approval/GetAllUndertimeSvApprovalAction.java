...
@WebServlet("/GetAllUndertimeSvApprovalAction")
public class GetAllUndertimeSvApprovalAction extends HttpServlet {
...
//		int supervisorId = Integer.parseInt(request.getParameter("superVisorId"));
		...
		List<EmployeeUndertime> list = null;
...
//			list = service.getAllOvertimeForSvApprovalBySuperVisorId(supervisorId, startPageIndex,numRecordsPerPage, sorting);
			...
			if("ADMIN".equals(approverType)){
				//get all Leave
				list = service.getAllUndertimeForApproval(employeeLoggedIn.getEmpId(), startPageIndex,numRecordsPerPage, sorting);
			} else if("HRADMIN".equals(approverType)) {
				//get all Leave base on sectionId
				list = service.getAllUndertimeForHRApproval(employeeLoggedIn.getEmpId(), startPageIndex,numRecordsPerPage, sorting);
			} else if("SECTIONADMIN".equals(approverType)) {
				//get all Leave base on sectionId
				list = service.getAllUndertimeForApprovalBySectionId(employeeLoggedIn.getEmpId(), employeeLoggedIn.getSectionId(), startPageIndex,numRecordsPerPage, sorting);
			} else if("UNITADMIN".equals(approverType)) {
				//get all Leave base on unitId
				list = service.getAllUndertimeForApprovalByUnitId(employeeLoggedIn.getEmpId(), employeeLoggedIn.getUnitId(), startPageIndex,numRecordsPerPage, sorting);
			} else if("SUBUNITADMIN".equals(approverType)) {
				//get all Leave base on subUnitId			
				list = service.getAllUndertimeForApprovalBySubUnitId(employeeLoggedIn.getEmpId(), employeeLoggedIn.getSubUnitId(), startPageIndex,numRecordsPerPage, sorting);
			}
...
//        response.getWriter().print(json);
		...
		JsonElement element = gson.toJsonTree(list,new TypeToken<List<EmployeeUndertime>>(){}.getType());
