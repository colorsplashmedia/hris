...
@WebServlet("/UpdateUndertimeSvApprovalAction")
public class UpdateUndertimeSvApprovalAction extends HttpServlet {
...
//		HttpSession session = request.getSession(true);   				
		...
		String empUndertimeId = request.getParameter("empUndertimeId");     
		IEmployeeUndertimeService service = new EmployeeUndertimeService();
        
		try {
			
			EmployeeUndertime param = new EmployeeUndertime();
			
			param.setApprovedBy(employeeLoggedIn.getEmpId());
			param.setEmpUndertimeId(Integer.parseInt(empUndertimeId));
			...
			service.approveUndertime(param);
